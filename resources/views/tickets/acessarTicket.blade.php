...
@section('conteudo')
@php
if($ticket->aluno->imagem == ""){
    $avatar = "/public/assets/img/avatars/1.png";
}
else{
    $avatar = "/public/img/alunos/".$ticket->aluno->imagem."?".date('YmdHis');
}
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-start align-items-center">
                <a href="{{ route('tickets') }}" title="Voltar" style="margin-right: 20px">
                    <img src="{{ asset('/public/img/IconsPng/Voltar.png') }}" height="50px" alt="">
                </a>
                <h5 class="card-title">Acessar Ticket</h5>
            </div>
            <div class="row mt-3">
                <div class="col-md-12">
                    <label for="">Aluno:</label><br>
                    <b>{{ $ticket->aluno->nmAluno }}</b>
                </div>
            </div>
            <div class="row mt-3">
                <div class="col-md-6">
                    <label for="">Assunto:</label><br>
                    <b>{{ $ticket->dsAssunto }}</b>
                </div>
                <div class="col-md-3">
                    <label for="">Abertura</label><br>
                    <b>{{ $dtHrTicket }}</b>
                </div>
                <div class="col-md-3">
            ...
            @if($ticket->stTicket == "Aberto")
                <div class="row mt-5">
                    <div class="col-md-12">
                        <button type="button" id="fecharTicket" class="btn btn-danger">Fechar Ticket</button>
                    </div>
                </div>
            @endif
        </div>
                ...
                <div class="chat-history-wrapper">
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="d-flex overflow-hidden align-items-center">
                                ...
                                <div class="chat-contact-info flex-grow-1 ms-3">
                                    <h6 class="m-0">{{ $ticket->aluno->nmAluno }}</h6>
                                    <span class="user-status">Mensagens do Ticket</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    @if($ticket->stTicket == "Aberto")
                        <div class="chat-history-footer">
                            <form action="{{ route('tickets.addMensagem') }}" class="form-send-message d-flex justify-content-between align-items-center" method="post">
                                @csrf
...
<script>
document.getElementById('fecharTicket').addEventListener('click', ()=>{
    if(confirm('Tem certeza que deseja fechar este ticket?')){
        window.location.href = "{{ route('tickets.encerrarTicket', $ticket->id) }}";
    }
})
</script>
